...
Uma expressão pode produzir um valor à partir de um literal, de uma variável, de uma chamada de função ou de uma operação sobre valores produzidos por sub-expressões.

- literal inteiro:
  - sequência de dígitos decimais iniciados por `1` a `9`: corresponde ao valor inteiro codificado em decimal. Tem o tipo `int` ou `long` ou `long long`, o menor que puder representar o valor. Se for seguido por `u` ou `U`, é `unsigned`. Se for seguido por `l` ou `L` é pelo menos `long` e se for seguido por `ll` ou `LL` é `long long`.
...
Os operadores em C podem ser unários (têm um só operando), binários (têm dois operandos) ou ternário (tem 3 operandos). Os operadores unários geralmente precedem seus operandos (mas existem dois operadores que ficam após o operando); os operadores binários são escritos entre seus operandos (como em `a + b`, em que o operador `+` opera sobre os operandos `a` e `b`). A maioria dos operadores opera sobre operandos e produz resultados de mesmo tipo.

Se for o caso de uma operação entre tipos diferentes, o valor do menor tipo é convertido para o maior, ou o valor inteiro é convertido para ponto flutuante antes da operação ser feita. Essas conversões são decididas para cada operação, na ordem em que são executadas.
...
- binários aritméticos — os operandos são valores de mesmo tipo, inteiro ou ponto flutuante, o resultado é do mesmo tipo dos operandos:
  - `+` — soma
  - `-` — subtração
  - `*` — multiplicação
  - `/` — divisão - se for inteira, o resultado é truncado
...
- unário de referência
  ...
  - `*` — obtém o valor referenciado pelo operador à direita, que deve ser uma referência (tipicamente o valor de um ponteiro)
...
Se for necessário converter tipos, essa decisão é tomada a cada operador, na ordem em que forem operados. Por exemplo, se `f` é `float` e `i` é `int`, a expressão `f = i / 2;` tem dois operadores, e `/` será operado antes de `=`. Os operandos de `/` são ambos inteiros, então a divisão será inteira, produzindo resultado inteiro. Depois, é operado o `=`, que fará a atribuição de um inteiro para uma variável `float`; antes da atribuição o valor inteiro é convertido para `float`.
